                ...
                <div key={settingId} className="space-y-1">
                    ...
                    {field.type === 'select' ? (
                        ...
                        <div className="flex items-center justify-between bg-[#162032] p-2 rounded border border-gray-700">
                            <span id={`label-${settingId}`} className="text-xs text-gray-300 font-ark">{t(`systemConfig.fields.${settingId}`, field.label)}</span>
                            <button
                                role="switch"
                                ...
                                className={`w-8 h-4 rounded-full relative transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ffd700] focus-visible:ring-offset-1 focus-visible:ring-offset-[#162032] ${config[settingId as keyof AppConfig] ? 'bg-green-500' : 'bg-gray-600'}`}
                            >
            ...
            <nav aria-label="Config Tabs" className="flex border-b-2 border-white bg-blue-900 scrollbar-hide overflow-x-auto">
                {tabs.map((tab: any) => (
                    <button
                        ...
                        className={`flex-1 py-3 px-4 flex items-center justify-center gap-2 font-ark text-xs transition-colors whitespace-nowrap ${activeTab === tab.id
                ...
                {activeTab === 'appearance' && (
                    <div className="space-y-6">
                        ...
                        <section className="space-y-3">
                            ...
                            <div className="flex items-center justify-between bg-[#162032] p-2 rounded border border-gray-700">
                                <span id="label-enable-audio" className="text-xs text-gray-300 font-ark">{t('appearance.enableAudio', 'Enable Audio (Video Only)')}</span>
                                <button
                                    role="switch"
                                    aria-checked={!!themeConfig?.userAssets?.startScreenAudio}
                                    aria-labelledby="label-enable-audio"
                                    onClick={() => onThemeConfigChange?.({
                                        ...themeConfig!,
                                        userAssets: { ...themeConfig?.userAssets, startScreenAudio: !themeConfig?.userAssets?.startScreenAudio }
                                    })}
                                    className={`w-8 h-4 rounded-full relative transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ffd700] focus-visible:ring-offset-1 focus-visible:ring-offset-[#162032] ${themeConfig?.userAssets?.startScreenAudio ? 'bg-green-500' : 'bg-gray-600'}`}
                                >
