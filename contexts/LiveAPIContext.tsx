...
export const LiveAPIProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
        ...
        switch (message.type) {
            ...
            case 'turn_complete':
                ...
                if (pendingReconnectRef.current) {
                    ...
                    if (latestConfigRef.current) {
                        // We use the full connect logic but it will handle the disconnect internally
                        // In the future this might call a cleaner reconnect function
                        connect(latestConfigRef.current);
                    }
                } else {
            ...
            case 'go_away':
                ...
                if (!isModelRespondingRef.current && !isSpeakingRef.current) {
                    ...
                    if (latestConfigRef.current) connect(latestConfigRef.current);
                } else {
    ...
    }, [connected, connecting, handleMessage]);
    ...
    const sendMessage = useCallback((text: string, config: AppConfig) => {
        if (clientRef.current) {
            ...
            try {
                if (screenSharingRef.current && screenCaptureRef.current) {
                    imageBase64 = screenCaptureRef.current.takeSnapshot().split(',')[1];
                } else if (videoStreamerRef.current) {
                    // Only fallback to videoStreamer if sharing isn't active, to avoid double capturing
