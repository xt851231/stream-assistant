...
export class GeminiFlashAdapter extends ModelAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            this.tts = this.TTSFactoryClass.createAdapter('gemini', this.config);
    ...
    updateConfig(config) {
        ...
        // If voice changed, update TTS
        if (config.voice && this.tts) {
            console.log(`🔄 Flash Adapter: Updating TTS voice to ${config.voice}`);
            // Re-create TTS adapter with new config
            // Note: This relies on TTSFactory creating a new instance. 
            // Ideally TTSAdapter should have setVoice, but recreating is safer for now.
    ...
    async sendAudioToFlash(base64WavAudio) {
        ...
        try {
            const contents = this._buildAudioRequestContents(base64WavAudio);
            ...
            // Process the stream and extract transcription/response
            const { fullText, transcription, responseText } = await this._handleAudioResponseStream(responseStream);

            // Speak full response as single TTS call for consistent voice
            await this._speakResponse(responseText.trim());
    ...
    /**
     * Helper to build contents for audio request
     */
    _buildAudioRequestContents(base64WavAudio) {
        const parts = [];
...
`       });
    ...
    /**
     * Helper to process the response stream and parse transcription
     */
    async _handleAudioResponseStream(stream) {
        let fullText = "";
        let transcription = "";
        let responseText = "";
        ...
        return { fullText, transcription, responseText };
    }
    ...
    async sendText(text, base64Image = null) {
        ...
        try {
            ...
            // Speak full response as single TTS call for consistent voice
            await this._speakResponse(fullText.trim());
    ...
    /**
     * Helper to get system instruction with affective settings
     */
    ...
    /**
     * Helper to speak response text with cool-down
     */
