...
export class GeminiLiveAdapter extends ModelAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            const sanitizedConnectConfig = { ...connectConfig };
            if (sanitizedConnectConfig.systemInstruction) {
                const len = sanitizedConnectConfig.systemInstruction.length;
                sanitizedConnectConfig.systemInstruction = `<redacted length=${len}>`;
            }

            console.log('🛠️ SETUP OBJECT (connectConfig):', JSON.stringify({
                model: this.model,
                config: sanitizedConnectConfig
            }, null, 2));
