...
export class GeminiLiveAdapter extends ModelAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            // Redact sensitive configuration before logging
            const safeConnectConfig = { ...connectConfig };
            if (safeConnectConfig.systemInstruction) {
                const len = safeConnectConfig.systemInstruction.length;
                safeConnectConfig.systemInstruction = `<redacted length=${len}>`;
            }

            console.log('🛠️ SETUP OBJECT (connectConfig):', JSON.stringify({
                model: this.model,
                config: safeConnectConfig
            }, null, 2));
    ...
    handleIncomingMessage(message) {
        ...
        if (serverContent) {
            ...
            if (serverContent.inputTranscription) {
                console.log('📝 Input transcription (serverContent):', `<redacted length=${serverContent.inputTranscription.length}>`);
                this.emit('content', {
            ...
            if (serverContent.outputTranscription) {
                console.log('📝 Output transcription (serverContent):', `<redacted length=${serverContent.outputTranscription.length}>`);
                this.emit('content', {
        ...
        if (message.inputTranscription && !serverContent?.inputTranscription) {
            console.log('📝 Input transcription (top-level):', `<redacted length=${message.inputTranscription.length}>`);
            this.emit('content', {
                type: 'input_transcription',
        ...
        if (message.outputTranscription && !serverContent?.outputTranscription) {
            console.log('📝 Output transcription (top-level):', `<redacted length=${message.outputTranscription.length}>`);
            this.emit('content', {
                type: 'output_transcription',
