...
export class GeminiLiveAdapter extends ModelAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            // If we have history to restore, send it immediately
            if (this.config.history && this.config.history.length > 0) {
                console.log(`📜 Restoring ${this.config.history.length} messages of history to new session...`);
                this.session.sendClientContent({
                    turns: this.config.history,
                    turnComplete: true
                });
            }
