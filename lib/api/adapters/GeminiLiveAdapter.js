...
export class GeminiLiveAdapter extends ModelAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            const connectConfigForLog = { ...connectConfig };
            if (connectConfigForLog.systemInstruction) {
                const sysInstStr = connectConfigForLog.systemInstruction;
                connectConfigForLog.systemInstruction = `<redacted length=${sysInstStr.length || 0}>`;
            }

            console.log('🛠️ SETUP OBJECT (connectConfig):', JSON.stringify({
                model: this.model,
                config: connectConfigForLog
            }, null, 2));
