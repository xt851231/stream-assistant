...
export class GeminiLiveAdapter extends ModelAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            console.log('🔧 FULL connectConfig:', JSON.stringify(connectConfig, null, 2));
    ...
    disconnect() {
        if (this.session) {
            ...
            const ws = this._getWebSocket(ws => typeof ws.close === 'function');
    ...
    async sendImage(base64Image, mimeType = "image/jpeg") {
        ...
        const ws = this._getWebSocket(ws => typeof ws.send === 'function');
    ...
    /**
     * Helper to retrieve the underlying WebSocket from the session.
     * This traverses known internal properties where the SDK might store the WebSocket.
     * @param {Function} [validator] - Optional predicate to validate the WebSocket candidate.
     * @private
     */
    _getWebSocket(validator) {
