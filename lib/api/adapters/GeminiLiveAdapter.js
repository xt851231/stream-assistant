...
export class GeminiLiveAdapter extends ModelAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            const loggableConfig = { ...connectConfig };
            if (loggableConfig.systemInstruction) {
                loggableConfig.systemInstruction = `<redacted length=${loggableConfig.systemInstruction.length}>`;
            }
    ...
    handleIncomingMessage(message) {
        ...
        if (serverContent) {
            ...
            if (serverContent.inputTranscription) {
                console.log('📝 Input transcription (serverContent):', `<redacted length=${serverContent.inputTranscription?.length || 0}>`);
                this.emit('content', {
            ...
            if (serverContent.outputTranscription) {
                console.log('📝 Output transcription (serverContent):', `<redacted length=${serverContent.outputTranscription?.length || 0}>`);
                this.emit('content', {
        ...
        if (message.inputTranscription && !serverContent?.inputTranscription) {
            console.log('📝 Input transcription (top-level):', `<redacted length=${message.inputTranscription?.length || 0}>`);
            this.emit('content', {
                type: 'input_transcription',
        ...
        if (message.outputTranscription && !serverContent?.outputTranscription) {
            console.log('📝 Output transcription (top-level):', `<redacted length=${message.outputTranscription?.length || 0}>`);
            this.emit('content', {
                type: 'output_transcription',
