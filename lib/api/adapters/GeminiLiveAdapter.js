...
export class GeminiLiveAdapter extends ModelAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            const logConfig = { ...connectConfig };
            if (logConfig.systemInstruction) {
                const len = typeof logConfig.systemInstruction === 'string'
                    ? logConfig.systemInstruction.length
                    : (logConfig.systemInstruction.parts?.[0]?.text?.length || 0);
                logConfig.systemInstruction = '<redacted length=' + len + '>';
            }
