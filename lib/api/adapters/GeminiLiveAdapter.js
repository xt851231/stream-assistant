...
export class GeminiLiveAdapter extends ModelAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            const loggedConfig = JSON.parse(JSON.stringify(connectConfig));
            if (loggedConfig.systemInstruction) {
                if (loggedConfig.systemInstruction.parts && loggedConfig.systemInstruction.parts[0]) {
                    loggedConfig.systemInstruction.parts[0].text = `<redacted length=${loggedConfig.systemInstruction.parts[0].text?.length || 0}>`;
                } else {
                    loggedConfig.systemInstruction = `<redacted length=${loggedConfig.systemInstruction?.length || 0}>`;
                }
            }
    ...
    setHistory(messages) {
        ...
        console.log("📝 History Text Payload:\n", `<redacted length=${historyText?.length || 0}>`);
