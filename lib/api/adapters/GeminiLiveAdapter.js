...
export class GeminiLiveAdapter extends ModelAdapter {
    ...
    async connect(config) {
        ...
        try {
            ...
            const redactedConfig = { ...connectConfig };
            if (redactedConfig.systemInstruction) {
                redactedConfig.systemInstruction = `<redacted length=${redactedConfig.systemInstruction.length || JSON.stringify(redactedConfig.systemInstruction).length}>`;
            }

            console.log('🛠️ SETUP OBJECT (connectConfig):', JSON.stringify({
                model: this.model,
    ...
    _buildConnectConfig() {
        ...
        // Setup sessionResumption to receive handles, and optionally resume if we have one
        connectConfig.sessionResumption = {};
        if (this.config.sessionHandle) {
            connectConfig.sessionResumption.handle = this.config.sessionHandle;
            console.log(`🔄 Attempting to resume session with handle: ${this.config.sessionHandle}`);
        }
    ...
    disconnect() {
        if (this.session) {
            ...
            if (ws && typeof ws.close === 'function') {
                try {
                    // Only invoke close if it's currently OPEN (1) to prevent "already closing" DOMExceptions
    ...
    /**
     * Set chat history for the live session context.
     * Compiles the array of previous messages into a single narrative text block
     * and injects it as a single 'user' turn to avoid role validation issues.
     * @param {Array} messages 
     */
    setHistory(messages) {
        if (!this.session || !messages || messages.length === 0) return;

        // Skip history injection if we are resuming a session.
        // Session handles naturally restore context on the server side.
        if (this.config.sessionHandle) {
            console.log("🔄 Session resumption active. Skipping manual history injection.");
            return;
        }
        ...
        let historyText = "[System Note: Previous Conversation History]\n";
        for (const msg of validMessages) {
            const sender = msg.sender || (msg.type === 'assistant' ? 'Model' : 'User');
            historyText += `[${sender}]: ${msg.text}\n`;
        }
        historyText += "[End of Previous History, New Session Begins]";

        console.log(`📜 Injecting ${validMessages.length} history turns as a single context block into Live Session...`);
        console.log("📝 History Text Payload:\n", `<redacted length=${historyText.length}>`);
    ...
    handleIncomingMessage(message) {
        ...
        if (serverContent) {
            ...
            if (serverContent.sessionResumptionUpdate) {
                if (serverContent.sessionResumptionUpdate.resumable && serverContent.sessionResumptionUpdate.newHandle) {
                    this.emit('content', {
                        type: 'session_resumption',
                        data: serverContent.sessionResumptionUpdate.newHandle
                    });
                }
            }

            if (serverContent.inputTranscription) {
                console.log('📝 Input transcription (serverContent):', `<redacted length=${JSON.stringify(serverContent.inputTranscription).length}>`);
                this.emit('content', {
            ...
            if (serverContent.outputTranscription) {
                console.log('📝 Output transcription (serverContent):', `<redacted length=${JSON.stringify(serverContent.outputTranscription).length}>`);
                this.emit('content', {
        ...
        if (message.inputTranscription && !serverContent?.inputTranscription) {
            console.log('📝 Input transcription (top-level):', `<redacted length=${JSON.stringify(message.inputTranscription).length}>`);
            this.emit('content', {
                type: 'input_transcription',
        ...
        if (message.outputTranscription && !serverContent?.outputTranscription) {
            console.log('📝 Output transcription (top-level):', `<redacted length=${JSON.stringify(message.outputTranscription).length}>`);
            this.emit('content', {
                type: 'output_transcription',
        ...
        // Handle sessionResumptionUpdate that might arrive at root level instead of serverContent
        if (message.sessionResumptionUpdate && !serverContent?.sessionResumptionUpdate) {
            if (message.sessionResumptionUpdate.resumable && message.sessionResumptionUpdate.newHandle) {
                this.emit('content', {
                    type: 'session_resumption',
                    data: message.sessionResumptionUpdate.newHandle
                });
            }
        }

        // Handle Server Drains (GoAway) safely
        if (message.goAway) {
            console.warn(`🔄 Preemptive [goAway] signal received from Gemini Live API. Reconnection required soon.`);
            this.emit('content', {
                type: 'go_away',
                data: message.goAway
            });
        } else if (serverContent && serverContent.goAway) {
            console.warn(`🔄 Preemptive [goAway] signal received from Gemini Live API via serverContent. Reconnection required soon.`);
            this.emit('content', {
                type: 'go_away',
