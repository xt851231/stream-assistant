...
import { base64ToUint8Array } from '../../../utils/base64-utils.js';
/**
...
export class GeminiTTSAdapter extends TTSAdapter {
    constructor(config) {
        ...
        // Use ttsVoice if provided, fallback to persona voice
        const voiceName = config.ttsVoice || config.voice;
        this.voiceConfig = voiceName ? { prebuiltVoiceConfig: { voiceName: voiceName } } : undefined;
    }
    ...
    pcmToAudioBuffer(pcmBytes) {
        ...
        if (pcmBytes.byteOffset % 2 === 0) {
            ...
            int16View = new Int16Array(pcmBytes.buffer, pcmBytes.byteOffset, numSamples);
        } else {
            // Unaligned: must copy to a new aligned buffer
            // We slice only the valid bytes (numSamples * 2) to ensure even length
            const alignedBuffer = pcmBytes.slice(0, numSamples * 2).buffer;
            int16View = new Int16Array(alignedBuffer);
        }

        const float32Data = new Float32Array(numSamples);
        const inv32768 = 1 / 32768; // Multiply is faster than divide

        for (let i = 0; i < numSamples; i++) {
            // Normalize 16-bit signed int (-32768 to 32767) to float (-1.0 to ~1.0)
            float32Data[i] = int16View[i] * inv32768;
        }
