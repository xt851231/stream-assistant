...
export class AudioStreamer {
  constructor(geminiClient) {
    ...
    this.vadThreshold = 0.02; // Reduced back to 0.02 to capture word beginnings faster
        ...
        if (event.data.type === "audio") {
          ...
          if (this.vadEnabled) {
            ...
            if (currentlySpeaking) {
              ...
              // Maintain padding buffer size (1024 samples = 64ms per chunk at 16kHz)
              // Each chunk is 1024 samples. 1024 / 16000 = ~0.064s or 64ms
              const maxPaddingChunks = Math.ceil(this.prefixPadding / 64);
  ...
  async stop() {
    ...
    if (this.audioWorklet) {
      try {
        ...
        console.warn("Soft fail disconnecting audio worklet:", e);
      }
