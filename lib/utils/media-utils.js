    ...
    try {
      ...
      this.audioWorklet.port.onmessage = (event) => {
        ...
        if (event.data.type === "audio") {
          ...
          const base64Audio = base64Utils.arrayBufferToBase64(pcmData);
            ...
            if (this.isSpeaking !== currentlySpeaking) {
              ...
              if (!currentlySpeaking && this.client && this.client.connected) {
                ...
                if (typeof this.client.onSpeechEnd === 'function') {
                  ...
                  const base64Silence = base64Utils.arrayBufferToBase64(silenceBuffer);
...
export class AudioPlayer {
  ...
  async play(base64Audio) {
    ...
    try {
      ...
      const arrayBuffer = base64Utils.base64ToArrayBuffer(base64Audio);

      // Convert PCM16 LE to Float32
      const inputArray = new Int16Array(arrayBuffer);
      const len = inputArray.length;
      const float32Data = new Float32Array(len);
      const scalar = 1 / 32768;

      for (let i = 0; i < len; i++) {
        float32Data[i] = inputArray[i] * scalar;
      }
